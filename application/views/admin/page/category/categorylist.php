		...
		<div class="modal-dialog center modal-lg">
			<div class="modal-content animated">
				...
				<div class="form-group row" style="margin-top: 15px;">
					...
					<div class="col-sm-8">
						<input type="text" id="category_name" style="width: 100%;" placeholder="" required="" name="category_name" autofocus="1" autocomplete="off">
				...
				<div class="form-group row" style="margin-top: 15px;">
					...
					<div class="col-sm-8">
						<input type="text" id="service_charge" style="width: 100%;" placeholder="" required="" name="service_charge" autofocus="1" autocomplete="off">
					</div>
				</div>
...
<script>
	function getsId(category_id) {
		$.ajax({
			url: "<?php echo base_url(); ?>category/category/edit_category_frm?category_id=" + category_id,
			type: "GET",
			dataType: 'json',
			success: function(json) {
				$('#category_name').val(json.category_name);
				$('#service_charge').val(json.service_charge);
	...
	function updateEntryFrom() {
		...
		if (statuschk !== "" && category_name !== "") {
			$.ajax({
				type: "POST",
				dataType: 'json',
				url: "<?php echo base_url(); ?>mangement/mangement/insertCategory",
				cache: false,
				data: dataString,
	...
	function filterCategory() {
		...
		$.ajax({
			url: "<?php echo base_url(); ?>/category/Category/getJsonCategory?name=" + value + "&status=" + sts,
			type: "GET",
			dataType: 'json',
			success: function(json) {
				$('#loading').html(json);
				$.map(json, function(item) {
					...
					html += "<td class='text-left'>" + item.service_charge + "</td>";
