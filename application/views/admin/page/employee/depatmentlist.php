		...
		<div class="card">
			<div class="card-body">
				<div class="row">
					...
					<div class="col-md-6">
						<div align="right"><button type="button" class="btn btn-primary" onclick="showModal();">Add New Depatment</button></a></div><br>
					</div>
				</div>
				<div class="row">
					<div class="col-8">
		...
		<div class="modal-dialog center modal-lg">
			<div class="modal-content animated">
				...
				<div class="form-group row" style="margin-top: 15px;">
					...
					<div class="col-sm-8">
						<input type="text" class="form-control" id="dpt_name" placeholder="" required="" name="dpt_name" autofocus="1" autocomplete="off">
...
<script>
	function updateEntryFrom() {
		...
		if (statuschk !== "" && dpt_name !== "") {
			$.ajax({
				type: "POST",
				dataType: 'json',
				url: "<?php echo base_url(); ?>mangement/mangement/insertDepartment",
				cache: false,
				data: dataString,
			...
			alert("Please write depatment name");
		}
	...
	function getsId(dpt_id) {
		$.ajax({
			url: "<?php echo base_url(); ?>employee/employee/edit_dpt_response?dpt_id=" + dpt_id,
			type: "GET",
			dataType: 'json',
			success: function(json) {
				$('#dpt_name').val(json.dpt_name);
	...
	function filterDpt() {
		...
		$.ajax({
			url: "<?php echo base_url(); ?>/employee/employee/getJsondpt?name=" + value + "&status=" + sts,
			type: "GET",
			dataType: 'json',
			success: function(json) {
				$('#loading').html(json);
				$.map(json, function(item) {
					var id = "item_list" + item.dpt_id;
					var html = "<tr id='" + id + "'>";
					html += "<td>" + sl + "</td>";
					html += "<td class='text-left'>" + item.dpt_name + "</td>";
					html += "<td class='text-left'>" + statuss[item.status] + "</td>";
					html += "<td class='text-left' onclick=getsId(" + item.dpt_id + ")>" + 'Edit' + "</td>";
					html += "</tr>";
					if ($('#' + id).length < 1) {
