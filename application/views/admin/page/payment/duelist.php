...
<input type="hidden" name="customer_due_id" id="customer_due_id" />
						<div class="form-group">
...
<script>
	function storeEntryFrom() {
		...
		if (user_id !== '' && amount !== '' && typeofpayment !== '') {
			$.ajax({
				type: "POST",
				dataType: 'json',
				url: "<?php echo base_url(); ?>mangement/mangement/save_due",
				data: dataString,
	...
	function filterBooking() {
		...
		$.ajax({
			url: "<?php echo base_url(); ?>customer_due/Customerdue/getcustomerdues?f_date=" + f_date +
				"&t_date=" + t_date,
			type: "GET",
			dataType: 'json',
			success: function(json) {
				...
				$.map(json, function(item) {
					...
					var id = "item_list" + item.customer_due_id;
					var html = "<tr id='" + id + "'>";
					html += "<td ><input type='checkbox' checkbox name='bookingId[]' value='" + item.customer_due_id + "'>" + "</td>";
					html += "<td class='text-left' onclick=getsId(" + item.customer_due_id + ")>" + sl + "</td>";
					html += "<td class='text-left' onclick=getsId(" + item.customer_due_id + ")>" + item.entry_date + "</td>";
					html += "<td class='text-left' onclick=getsId(" + item.customer_due_id + ")>" + item.company + "</td>";
					html += "<td class='text-left' onclick=getsId(" + item.customer_due_id + ")>" + item.mobile_number + "</td>";
					html += "<td class='text-left' onclick=getsId(" + item.customer_due_id + ")>" + item.amount + "</td>";
					html += "<td class='text-left' onclick=getsId(" + item.customer_due_id + ")>" + 'Edit' + "</td>";
	...
	function getsId(customer_due_id) {
		$.ajax({
			url: "<?php echo base_url(); ?>customer_due/Customerdue/geteditcustomerdueData?customer_due_id=" + customer_due_id,
			type: "GET",
			dataType: 'json',
			success: function(json) {
				$('#ratetype').val(json.rate_type);
				$('#user_id').val(json.user_id);
				$('#customer_due_id').val(json.customer_due_id);
